...
class kenel_shap():
    def __init__(self, model, explain_data, bg_data, logger, use_gpu,
        ...
        self.bg_data_id = bg_model_input[1]

        self.bg_data_val = bg_model_input[2]
    ...
    def explain(self, instance, n_samples):
        ...
        print("sample shap cost {}s, run instance cost {}s".format((t2 - t1), (t3 - t2)))
        return phi
    ...
    def addsample(self, x, m, w):
        ...
        if offset < self.seq_masks.shape[0]:
            ...
            seq_tss = x['seq_tss']
            pre_id = x['pred_evt_ids']
            pre_value = x['pred_evt_values']
            self.seq_masks[offset] = seq_mask
            self.seq_lengths[offset] = seq_len
            self.seq_tss[offset] = seq_tss
            self.pre_ids[offset] = pre_id
            self.pre_values[offset] = pre_value
    ...
    def allocate(self, mode):
        ...
        self.pre_ids = torch.zeros((self.max_batch_size, self.num_event_feats, self.n_end - self.c_begin))
        self.pre_values = torch.zeros((self.max_batch_size, self.num_event_feats, self.n_end - self.c_begin))
        ...
        while used_sample < self.nsamplesAdded:
            ...
            if used_sample < self.nsamplesAdded and used_sample + batch_size > self.nsamplesAdded:
                model_input = [
                    ...
                    self.pre_ids[0: self.nsamplesAdded - used_sample],
                    self.pre_values[0: self.nsamplesAdded - used_sample],
                    self.seq_lengths[0: self.nsamplesAdded - used_sample]
                ...
                model_input = [
                    ...
                    self.pre_ids[0: batch_size],
                    self.pre_values[0: batch_size],
    ...
    def make_bg_input(self, bg_data, explain_data, mask_size=150):
        if self.mode == 0:
            ...
            pre_id = explain_data['pred_evt_ids']
            pre_value = explain_data['pred_evt_values']
            length = explain_data['seq_lengths']

            # add a batch dim
            model_input = [mask, seq_ids, seq_values, ts, pre_id, pre_value, length]
            return model_input
